...
const [code1, quantity1, price1] = lines[0].split(" ").map(Number);
const [code2, quantity2, price2] = lines[1].split(" ").map(Number);

const totalAmount = quantity1 * price1 + quantity2 * price2;
