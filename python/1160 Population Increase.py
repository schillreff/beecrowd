...
# know the exact time for these cases. In this way, for these test cases, it
# is enough printing the message "Mais de 1 seculo", that means "More than a
# Century".
...
# PA and PB (100 ≤ PA < 1000000, 100 ≤ PB ≤ 1000000, PA < PB) indicating
# respectively the population of A and B and two numbers G1 and G2 (0.1 ≤
# G1 ≤ 10.0, 0.0 ≤ G2 ≤ 10.0, G2 < G1) with one digit after the decimal point
#  each, indicating the populational growing (in percentual) for A and B respectively.

# Pay attention please: The population always is an integer number. So, a
# growing of 2.5% over a population of 100 will result in 102 (instead of 102.5)
#  and a growing of 2.5% over a population of 1000 will result in 1025. In addition,
#  use double variables to the growing rate.

# Output
# Print, for each test case, how many years would be necessary to the city A
# became greater than the city B (in inhabitants). Remember that if this time
